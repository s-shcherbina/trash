...
import {
  ...
  CreatePostNegativeReactionResponseDto,
  CreatePostNegativeReactionRequestDto
...
class Post extends Controller implements PostController {
  ...
  public negativeReact = async (
    request: FastifyRequest
  ): Promise<CreatePostNegativeReactionResponseDto> => {
    const authUserId = (request.user as UserAuthResponse).id;
    const reaction = await this.#postService.setNegativeReaction(
      authUserId,
      request.body as CreatePostNegativeReactionRequestDto
    );
